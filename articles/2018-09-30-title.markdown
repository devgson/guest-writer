...
In summary, you should go for the custom auth approach if your application requires a more robust and complex authentication approach which is usually the case. Thankfully, Firebase provides a way of integrating other authentication solutions into their services, you'll go through it in detail in this article.
...
* In your [Firebase Console](https://console.firebase.google.com/), click on the **Database** tab,
* Then click on the **Create Database** button, select the **Start in locked mode** option and create the database.
...
* Head over to your [Firebase Console](https://console.firebase.google.com/), click on the little gear icon  besides `Project Overview` and then click on **Project Settings**, you'd be redirected to the project **settings** page, when that loads, navigate to the **Service Accounts** tab.
...
* Click on the **New Application** button and give your application a suitable name, set the application type to **Regular Web Application** and then click on the **Create** button.
* You'd be redirected to the page of the newly created application, go the **Settings** tab of the application and copy the **Client ID**, **Client Secret** and **Domain** config values, you'll use them later on to initialize your Auth0 application on the client.
* Lastly, there's an **Allowed Callback URL** field in your application settings, this field specifies a list of allowed URL's users can be redirected to after authentication. You'll want to redirect authenticated users to `localhost:3000/callback` so add that URL to the field.
...
When that's done, run the following command in the terminal in the application directory:
...
Now you'll initialize  Auth0 on the client. Create a folder called `app` in the `public` directory and then create a file called `auth0.js` in the `app` folder . This is where you'll initialize and configure Auth0. Paste the code below into this file and we'll explore the contents right after:
...
Open up your terminal in your project directory and set your project environmental variables. Replace the values below with your Auth0 application's `domain` and `audience` values.
...
Now you can open `localhost:3000` to see the chat application, sign in and try it out.
